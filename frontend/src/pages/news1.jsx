...
const NEWS_SLUG = "news-1";
const NEWS_TITLE = "The Journey of Rice – From Farm to Global Markets";
        ...
        <div className="news-detail-wrapper">
          <div className="news-detail-main">
            <div className="news-detail-img-wrapper">
              <img src="/blog1.jpg" alt="Rice Farm" className="news-detail-img" />
            ...
            <h1 className="news-detail-title">{t('news_section.n1_title')}</h1>

            <div className="news-detail-content">
              <p>{t('news_articles.news1.p1')}</p>
              <p>{t('news_articles.news1.p2')}</p>
              <p>{t('news_articles.news1.p3')}</p>
            </div>

            <div className="news-related-box">
              <Link to="/news/news-2">{t('news_section.n2_title')}</Link>
            </div>

            {/* Approved Comments Display */}
            {comments.length > 0 && (
              <div className="news-comments-list" style={{ marginTop: '30px', marginBottom: '10px' }}>
                ...
                {comments.map((c) => (
                  <div key={c._id} style={{ borderBottom: '1px solid #eee', paddingBottom: '15px', marginBottom: '15px' }}>
                    ...
                    <span style={{ color: '#999', fontSize: '0.85rem', marginLeft: '12px' }}>
            ...
            <div className="news-comment-section">
              ...
              <form onSubmit={handleSubmit}>
                <div className="news-comment-row">
                  <input type="text" name="name" placeholder={t('news_page.full_name')} required
                    value={commentForm.name} onChange={handleChange} />
                  <input type="email" name="email" placeholder={t('news_page.email_address')} required
                    value={commentForm.email} onChange={handleChange} />
                </div>
                <textarea name="comment" placeholder={t('news_page.write_comment')} rows={6} required
                  value={commentForm.comment} onChange={handleChange}></textarea>
                <button type="submit" className="news-post-btn" disabled={submitting}>
                  {submitting ? t('news_page.posting') : t('news_page.post_comment')}
                </button>
              </form>
            </div>
          </div>
          ...
          <div className="news-detail-sidebar">
            ...
            <div className="sidebar-box">
              ...
              <div className="sidebar-latest-post">
                ...
                <div>
                  <span><FaUser /> {t('news_page.by')} {t('news_articles.author_aliha')}</span>
                  <Link to="/news/news-1">{t('news_section.n1_title').substring(0, 25)}...</Link>
                </div>
              </div>
              <div className="sidebar-latest-post">
                ...
                <div>
                  <span><FaUser /> {t('news_page.by')} {t('news_articles.author_aliha')}</span>
                  <Link to="/news/news-2">{t('news_section.n2_title').substring(0, 25)}...</Link>
                </div>
              </div>
              <div className="sidebar-latest-post">
                ...
                <div>
                  <span><FaUser /> {t('news_page.by')} {t('news_articles.author_aliha')}</span>
                  <Link to="/news/news-3">{t('news_section.n3_title').substring(0, 25)}...</Link>
                </div>
              </div>
            </div>

            <div className="sidebar-box">
              <h3>{t('news_page.categories')}</h3>
              <p className="sidebar-empty">{t('news_page.uncategorized')}</p>
            </div>

            <div className="sidebar-box">
              <h3>{t('news_page.tags')}</h3>
